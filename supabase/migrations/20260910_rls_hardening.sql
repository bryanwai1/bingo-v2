-- Section 4/6 of supabase/migrations/20260909_combined_catchup.sql
-- (20260703_multitenant_rls.sql), extracted to its own file for a deliberate,
-- separate run once sections 1,2,3,5,6 (already applied) are confirmed
-- working. One change from the original: its own bingo_can_write()
-- redefinition was removed — see the note at "── 0. Helper" below — because
-- the facilitator-aware version from section 5 is already live and would
-- otherwise be regressed back to the pre-facilitator one.
-- ============================================================================
...
-- ── 0. Helper ───────────────────────────────────────────────
-- bingo_can_write(row_owner) is intentionally NOT redefined here. This
-- section's own copy was the pre-facilitator version (owner-or-self only);
-- the facilitator-aware version from 20260704_facilitators.sql is already
-- installed and everything below only CALLS bingo_can_write(), so it works
-- unchanged. Redefining it here would silently regress facilitator writes.
