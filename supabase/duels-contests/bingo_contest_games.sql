-- bingo_contest_games: the mini-games playable inside a duel (bingo_duels),
-- authored from the admin instead of hardcoded in src/lib/contestGames.ts.
-- Source: duels-contests/003_contest_games.sql.

create table public.bingo_contest_games (
  id            uuid primary key default gen_random_uuid(),
  -- No FK to auth.users, unlike every other owner_id in this schema —
  -- carried over as-is from the original migration.
  owner_id      uuid,
  key           text not null,
  name          text not null,
...
create index bingo_contest_games_owner_idx on public.bingo_contest_games(owner_id);
create unique index bingo_contest_games_owner_key
  on public.bingo_contest_games(coalesce(owner_id,'00000000-0000-0000-0000-000000000000'::uuid), key);
...
-- Seed: the original hardcoded "Speed Edit Competition" game.
