...
function BingoTile({
  ...
  task: BingoTask
  status: TileStatus
  /** A submission waiting on the admin, or the newest one rejected. */
  review?: ReviewFlag
  /** A chained card whose prerequisite this team has not finished. Still
   *  tappable — the card explains itself — but greyed with a padlock. */
  chained: boolean
    ...
    >
      ...
      {review === 'pending' && status !== 'completed' && (
        <div className="absolute inset-0 bg-black/25 flex items-center justify-center z-20" title="Waiting for review">
          <div className="bg-amber-400 rounded-full w-7 h-7 flex items-center justify-center shadow">
            <span className="text-sm leading-none">⏳</span>
          </div>
        </div>
      )}
      {review === 'rejected' && status !== 'completed' && (
        <div className="absolute inset-0 bg-black/25 flex items-center justify-center z-20" title="Rejected - redo">
          <div className="bg-red-500 rounded-full w-7 h-7 flex items-center justify-center shadow">
